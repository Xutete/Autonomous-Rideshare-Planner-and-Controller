...
function [OptimalTour,mincost]=OptimizeRoute(cities, Dmatrix)
...
lastcity=length(cities);

NumOfDataSets=1;
for i=2:NumOfCities-1
    NumOfDataSets=NumOfDataSets+nchoosek(NumOfCities-1,i);
end
...
IndexLastStep=NumOfCities-1; %index into Data that marks the end of the previous step
CurrentData=IndexLastStep; %index into Data that marks the current dataset

%This is the main Dynamic Programming loop
for s=3:NumOfCities-1
    %generate possible sets with s-2 cities out of the possible N-2 cities
    %(not including city 1, last city)
    TempSets=nchoosek(2:NumOfCities-1,s-1);
    ...
    for j=1:NumOfSets(1) %iterate through all the sets
        for k=1:NumOfSets(2) %iterate through all the elements in each set
            SminuskSet=[6,TempSets(j,1:k-1),TempSets(j,k+1:NumOfSets(2))]; %this is the set S-{k}               
...
mm=0;
%Now add the distance back from the last city to destination city 
for i=IndexStartPrevStep:IndexLastStep
    mm=mm+1;
    candidatecost(mm)=Data(i).cost+D(Data(i).l,lastcity);
end
...
OptimalTour=[OptimalTour,lastcity];
