function [dist,path] = dijkstra_final(nodes,segments,start_id,finish_id,obstacle_nodes)
...
if (nargin < 3) % SETUP
    ...
    [distance,path] = dijkstra_new(nodes,segments,start_id,finish_id);
    ...
    %goes into here
        ...
        %zz is finishing node
        zz = find(finish_id == node_ids);
    end
    %while_cmd is not turned on for final node
    while eval(while_cmd)
        ...
        for k = 1:length(neighbor_ids)
            ...
            if ~settled(cidx)
                %nodes(pidx,2:cols);
                %nodes(cidx,2:cols);
                %here is where distance is calculated
                %length(obstacle_nodes);
%                 for(i=1:length(obstacle_nodes))
%                     nodes(obstacle_nodes(i),2)=nodes(obstacle_nodes(i),2)+20;
%                     nodes(obstacle_nodes(i),3)=nodes(obstacle_nodes(i),3)+20;
%                 end
